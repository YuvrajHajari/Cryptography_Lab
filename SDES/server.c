...
void decrypt(int *ct,int *k,int *pt){
    int k1[8],k2[8],ip[8];
    keygen(k,k1,k2);
    perm(ct,ip,IP,8);
    fk(ip,k2);
...
int main(){
    ...
    int blocks, bitlen;
    recv(client,(char*)&blocks,sizeof(int),0);
    recv(client,(char*)&bitlen,sizeof(int),0);
    ...
    for(int b=0;b<blocks;b++){
        printf("Block %d: ", b+1);
        for(int i=0;i<8;i++) printf("%d", buffer[b][i]);
        printf("\n");
    }
    ...
    int printed = 0;
    printf("\nDecrypted Plaintext: ");
    for(int b=0;b<blocks && printed<bitlen;b++){
        int pt[8];
        decrypt(buffer[b],key,pt);
        for(int i=0;i<8 && printed<bitlen;i++){
            printf("%d", pt[i]);
            printed++;
        }
    }
